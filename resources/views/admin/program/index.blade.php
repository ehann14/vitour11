...
<head>
    ...
    <title>Kelola Program Keahlian - Admin</title>
        ...
        <div class="row">
            ...
            <div class="col-md-2 sidebar p-0">
                ...
                <nav class="mt-3 p-2">
                    ...
                    <a href="{{ route('admin.program.index') }}" class="{{ request()->routeIs('admin.program.*') ? 'active' : '' }}"><i class="fas fa-layer-group"></i><span>Kelola Program</span></a>
            ...
            <div class="col-md-10">
                <nav class="navbar-admin">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 style="color: var(--primary-blue); margin: 0;"><i class="fas fa-layer-group me-2"></i>Kelola Program Keahlian</h4>
                            ...
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            @if($program->logo)
                                            <img src="{{ asset('storage/' . $program->logo) }}" alt="{{ $program->nama }}" style="width: 60px; height: 40px; object-fit: cover; border-radius: 6px;">
                                            @else
                                            <div style="width: 60px; height: 40px; background: #e9ecef; border-radius: 6px; display: flex; align-items: center; justify-content: center;"><i class="fas fa-image text-muted"></i></div>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="fw-bold">{{ $program->nama }}</div>
                                            <small class="text-muted">#{{ $program->id }}</small>
                                        </td>
                                        <td><span class="badge bg-primary">{{ $program->singkatan }}</span></td>
                                        <td>{{ $program->konsentrasi_count ?? 0 }}</td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="d-flex gap-1">
                                                <a href="{{ route('admin.program.edit', $program) }}" class="btn-action btn-edit"><i class="fas fa-edit"></i></a>
                                                <form action="{{ route('admin.program.destroy', $program) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus program ini?')">
                                                    @csrf @method('DELETE')
                                    ...
                                    @empty
                                    <tr><td colspan="6" class="text-center py-5 text-muted"><i class="fas fa-layer-group fa-3x mb-3 d-block opacity-25"></i>Belum ada program keahlian<br><a href="{{ route('admin.program.create') }}" class="btn btn-sm btn-primary mt-3">Tambah Pertama</a></td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
