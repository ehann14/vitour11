...
<body>
                ...
                <nav class="navbar-admin">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <h4 class="mb-0 fw-bold" style="color: var(--primary-blue);"><i class="fas fa-layer-group me-2"></i>Kelola Program Keahlian</h4>
                        </div>
                        <a href="{{ route('admin.program.create') }}" class="btn-primary-custom"><i class="fas fa-plus"></i>Tambah Baru</a>
                    </div>
                </nav>
                <div class="p-4">
                    ...
                    <div class="section-card">
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0">
                                <thead><tr><th width="70">Logo</th><th>Nama Program</th><th width="100">Singkatan</th><th width="100">Status</th><th width="140">Aksi</th></tr></thead>
                                <tbody>
                                    @forelse($programs as $item)
                                    <tr>
                                        <td>@if($item->logo)<img src="{{ asset('storage/' . $item->logo) }}" alt="{{ $item->nama }}" class="preview-program">@else<div class="preview-program d-flex align-items-center justify-content-center bg-light"><i class="fas fa-layer-group text-muted"></i></div>@endif</td>
                                        <td><div class="fw-bold">{{ $item->nama }}</div><small class="text-muted">Slug: {{ $item->slug }}</small></td>
                                        <td><span class="badge bg-primary bg-opacity-10 text-primary">{{ $item->singkatan }}</span></td>
                                        ...
                                        <td>
                                            <div class="d-flex gap-1">
                                                <a href="{{ route('admin.program.edit', $item) }}" class="btn-action btn-edit" title="Edit"><i class="fas fa-edit"></i></a>
                                                <form action="{{ route('admin.program.toggle-status', $item) }}" method="POST" class="d-inline">@csrf<button type="submit" class="btn-action btn-toggle {{ $item->is_active ? 'active' : '' }}" title="{{ $item->is_active ? 'Nonaktifkan' : 'Aktifkan' }}"><i class="fas fa-toggle-{{ $item->is_active ? 'on' : 'off' }}"></i></button></form>
                                                <form action="{{ route('admin.program.destroy', $item) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus program ini?')">@csrf @method('DELETE')<button type="submit" class="btn-action btn-delete" title="Hapus"><i class="fas fa-trash"></i></button></form>
                                            </div>
                                        </td>
                                    </tr>
                                    @empty
                                    <tr><td colspan="5" class="text-center py-5"><div class="empty-state"><i class="fas fa-layer-group"></i><p class="mb-0">Belum ada data program</p><a href="{{ route('admin.program.create') }}" class="btn btn-primary btn-sm mt-3">Tambah Pertama</a></div></td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
                    </div>
                    @if($programs->hasPages())<div class="d-flex justify-content-center mt-4">{{ $programs->onEachSide(1)->links('pagination::bootstrap-5') }}</div>@endif
                </div>
            </div>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        document.querySelectorAll('.alert').forEach(alert => { setTimeout(() => { const bsAlert = new bootstrap.Alert(alert); bsAlert.close(); }, 5000); });
        const sidebar = document.querySelector('.sidebar'), overlay = document.getElementById('sidebarOverlay'), toggleBtn = document.getElementById('sidebarToggleBtn'), closeBtn = document.getElementById('sidebarCloseBtn');
