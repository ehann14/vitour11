...
<head>
    ...
    <title>Tambah Konsentrasi - Admin</title>
            ...
            <div class="col-md-10">
                <nav class="navbar-admin">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 style="color: var(--primary-blue); margin: 0;"><i class="fas fa-plus-circle me-2"></i>Tambah Konsentrasi Keahlian</h4>
                        ...
                        <form action="{{ route('admin.konsentrasi.store') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="program_keahlian_id" class="form-select" required>
                                    ...
                                    <option value="{{ $prog->id }}" {{ old('program_keahlian_id') == $prog->id ? 'selected' : '' }}>{{ $prog->nama }} ({{ $prog->singkatan }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Nama Konsentrasi *</label>
                                <input type="text" name="nama" class="form-control" value="{{ old('nama') }}" required placeholder="Contoh: Pembangunan Game">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Deskripsi</label>
                                <textarea name="deskripsi" class="form-control" rows="3" placeholder="Jelaskan konsentrasi ini...">{{ old('deskripsi') }}</textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Kompetensi yang Dipelajari</label>
                                <textarea name="kompetensi" class="form-control" rows="3" placeholder="Contoh: 1. Pemrograman Web&#10;2. Database Design&#10;3. UI/UX Design">{{ old('kompetensi') }}</textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Prospek Karir</label>
                                <textarea name="prospek_karir" class="form-control" rows="3" placeholder="Contoh: Game Developer, App Developer, dll">{{ old('prospek_karir') }}</textarea>
                            </div>
                            <div class="row mb-4">
                                <div class="col-md-6">
                            ...
                            <div class="d-flex gap-2 pt-3 border-top">
                                <button type="submit" class="btn-primary-custom"><i class="fas fa-save me-1"></i>Simpan Konsentrasi</button>
