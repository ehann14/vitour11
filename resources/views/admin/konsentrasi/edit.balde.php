...
<head>
    ...
    <title>Edit Konsentrasi - Admin</title>
        ...
        <div class="row">
            ...
            <div class="col-md-2 sidebar p-0">
                ...
                <nav class="mt-3 p-2">
                    ...
                    <a href="{{ route('admin.konsentrasi.index') }}" class="active"><i class="fas fa-sitemap"></i><span>Kelola Konsentrasi</span></a>
            ...
            <div class="col-md-10">
                <nav class="navbar-admin">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 style="color: var(--primary-blue); margin: 0;"><i class="fas fa-edit me-2"></i>Edit Konsentrasi Keahlian</h4>
                        <a href="{{ route('admin.konsentrasi.index') }}" class="btn-secondary-custom"><i class="fas fa-arrow-left"></i>Kembali</a>
                    </div>
                </nav>
                ...
                <div class="p-4">
                    ...
                    <div class="form-card">
                        <form action="{{ route('admin.konsentrasi.update', $konsentrasi) }}" method="POST">
                            @csrf @method('PUT')
                            <div class="mb-3">
                                <label class="form-label">Program Induk *</label>
                                <select name="program_keahlian_id" class="form-select" required>
                                    <option value="">Pilih Program</option>
                                    @foreach($programs as $prog)
                                    <option value="{{ $prog->id }}" {{ old('program_keahlian_id', $konsentrasi->program_keahlian_id) == $prog->id ? 'selected' : '' }}>{{ $prog->nama }} ({{ $prog->singkatan }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Nama Konsentrasi *</label>
                                <input type="text" name="nama" class="form-control" value="{{ old('nama', $konsentrasi->nama) }}" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Deskripsi</label>
                                <textarea name="deskripsi" class="form-control" rows="3">{{ old('deskripsi', $konsentrasi->deskripsi) }}</textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Kompetensi yang Dipelajari</label>
                                <textarea name="kompetensi" class="form-control" rows="3">{{ old('kompetensi', $konsentrasi->kompetensi) }}</textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Prospek Karir</label>
                                <textarea name="prospek_karir" class="form-control" rows="3">{{ old('prospek_karir', $konsentrasi->prospek_karir) }}</textarea>
                            </div>
                            <div class="row mb-4">
                                <div class="col-md-6">
                                    <label class="form-label">Urutan Tampilan</label>
                                    <input type="number" name="urutan" class="form-control" value="{{ old('urutan', $konsentrasi->urutan) }}" min="0">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Status</label>
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" name="is_active" value="1" id="isActive" {{ old('is_active', $konsentrasi->is_active) ? 'checked' : '' }}>
                                        <label class="form-check-label" for="isActive">Aktif (Tampilkan di website)</label>
                                    </div>
                                </div>
                            </div>
                            <div class="d-flex gap-2 pt-3 border-top">
                                <button type="submit" class="btn-primary-custom"><i class="fas fa-save me-1"></i>Update Konsentrasi</button>
                                <a href="{{ route('admin.konsentrasi.index') }}" class="btn-secondary-custom"><i class="fas fa-times me-1"></i>Batal</a>
                            </div>
                        </form>
                    </div>
                </div>
